...
filename="snl_no_graph_gini_by_qoi_realizations.xlsx"
...
sens_data_qoi=sens_data[qoi].set_index('parameter')
...
ax.set_title(f"Gini importance measure by DFN realization number.\n QoI: {qoi_dict[qoi]}", fontweight='bold')

#adjust the padding
fig.tight_layout()

#save figure with desired name, format, dpi. f'{<variable_name>}' for filename customization using previously defined variables
plt.savefig(f'6_{filename_prefix}_{qoi}_multi_ribbon_gini.png',dpi=600)