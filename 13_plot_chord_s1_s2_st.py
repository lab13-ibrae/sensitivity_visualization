###################
# chord diagram
# this script was adapted for sensitivity indices from https://github.com/fengwangPhysics/matplotlib-chord-diagram by Feng Wang
import os
...
s1_s2_df=sens_data_qoi.pivot_table(index='parameter1',columns='parameter2',values='S2',sort=False)
s1_s2_df.index.name = 'Index'
s1_s2_df=s1_s2_df[all_param_names]
s1_s2_df=s1_s2_df.fillna(s1_s2_df.T).fillna(0)
st_df=sens_data1[qoi][['parameter','ST']].set_index('parameter')
...
for i,par in enumerate (all_param_names):
    short_annot[i]=par
...
for i in range(len(all_param_names)):
    ...
    plt.annotate(short_annot[i],(nodePos_corr[i][0], nodePos_corr[i][1]), fontsize = 13, color='0.2', family='sans-serif',va='center',annotation_clip=False)
...
fig.suptitle(f"Sobol' indices (main, total and interactions).\nQoI: {qoi}",fontsize=13,fontweight='bold',y=0.92)
fig.savefig(f"13_{filename_prefix}_{qoi}_chord_s1_s2_st", dpi=600)
