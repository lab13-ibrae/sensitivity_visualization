...
for qoi in wb.sheetnames:
    ...
    data = ws.values
    cols = next(data)[0:]
    sens_data1[qoi]=pd.DataFrame(data, columns=cols)
...
for qoi in wb.sheetnames:
    ...
    data = ws.values
    cols = next(data)[0:]
    sens_data2[qoi]=pd.DataFrame(data, columns=cols)
...
for par in all_param_names:
    ...
    sens_data_qoi=pd.concat([sens_data_qoi,pd.DataFrame([self_s1])])

si_for_plot={}
si_for_plot['S1']=dict(zip(sens_data1[qoi].parameter, sens_data1[qoi].S1))
si_for_plot['ST']=dict(zip(sens_data1[qoi].parameter, sens_data1[qoi].ST))

si_for_plot['S2']={}
s2_df=sens_data2[qoi]
for param1 in all_param_names:
    part_df=s2_df[s2_df.parameter1==param1]
    for param2 in list(set(all_param_names) - set(part_df.parameter2)):
         part_df=pd.concat([part_df,pd.DataFrame([{'S2':0, 'parameter1':param1, 'parameter2':param2,}])],axis=0, ignore_index=True)
    si_for_plot['S2'][param1]=dict(zip(part_df.parameter2, part_df.S2))
####plotting
...
for i, text in enumerate(G.nodes()):
    ...
    plt.annotate(short_annot[text], position, fontsize = 13, color='0.2', family='sans-serif',va='bottom')
ax.axis('off')

fig.suptitle(f"Sobol' indices (main, total and interactions).\nQoI: {qoi}",fontsize=13,fontweight='bold',y=0.95)

#fig.tight_layout()

fig.savefig(f"10_{filename_prefix}_{qoi}_circular_s1_s2_st", dpi=600)
